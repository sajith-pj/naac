...
function Reports() {
  ...
  useEffect(() => {
    if (criteriaList.length > 0) {
      let crite = criteriaList.filter(
        (val) => val.main_id === selectedCriteria
      );
      ...
      axios
        .get("/criterion/reports", {
          params: {
            ...
            criterion_id: selectedForReport,
          },
        })
        .then((response) => {
          setLoader2(false);
  ...
  const downloadExcelCsv = (apiData, bookType) => {
    ...
    apiData.forEach(
      ({
        ...
        sheetData.push({
          ...
          Active: is_active ? "OK" : "NO",
          Created: created_at ? dateFormat(created_at, "dS, mmmm yyyy") : "",
          ...rest,
        });
      }
    );
    ...
    FileSaver.saveAs(data, selectedForReport, fileExtension); // file name => criterion
  };
  ...
  const crateImagePdf = async (reportData, type) => {
    ...
    pdf.output("save", `${selectedForReport}.pdf`);
  };
  ...
  const generatePdfWithText = async (imageUrls) => {
    ...
    for (const [i, url] of imageUrls.entries()) {
      ...
      doc.addImage(
        ...
        10,
        12,
        image_width - 5 * pixcel,
        image_height - 5 * pixcel
      );
  ...
  const pdfFullReport = (reportDetails, pdfImageOnly = false) => {
    ...
    if (pdfImageOnly) {
      window.open(
        `/pdf-image-report?criterion_id=${reportDetails?.criteria}&department=${department}&program${reportDetails?.program}&status${reportDetails?.status}&from_date${reportDetails?.from_date}&to_date${reportDetails?.to_date}&type${reportDetails?.type}`,
        "_blank"
      );
    } else {
      window.open(
        `/html-reports?criterion_id=${reportDetails?.criteria}&department=${department}&program${reportDetails?.program}&status${reportDetails?.status}&from_date${reportDetails?.from_date}&to_date${reportDetails?.to_date}&type${reportDetails?.type}`,
        "_blank"
      );
    }
  };
  ...
  if (criteriaList.length === 0) {
    return (
      ...
      >
        <span>No Active Criterion Found</span>
      </div>
    );
  }
  ...
  return (
    <Row className="reports">
      ...
      <Col xs="12">
        <div className="pagetitle">
          <span>Version: {ctx.permissions.report_version}</span>
        </div>
      </Col>
      ...
      <Col xs="12" className="mt-2">
        <div className="criterion">
          {criteriaList.length > 0
            ? criteriaList.map((val, k) => {
                return (
                  <button
                    className={`criterion-btn ${
                      selectedCriteria === val.main_id ? "active" : ""
                    }`}
                    key={k}
        ...
        <div className="criterion-mobile">
          ...
          >
            {criteriaList.length > 0
              ? criteriaList.map((val, ky) => {
                  return (
                    <option key={ky} value={val.main_id}>
                      Criteria {val.main_id}
                    </option>
                  );
                })
      ...
      {showModal && (
        ...
        >
                      ...
                      >
                        ...
                        {programsList.map((program, ky) => {
                          return program.is_enabled &&
                            ...
                            <option key={ky} value={program.id}>
                              {" "}
                              {program.name}{" "}
                            </option>
                          ) : (
                  ...
                  <Col xs="12">
                    <FormGroup>
                      ...
                      >
                        ...
                        <option value="pdfFullReport">
                          PDF - Full Report with Media{" "}
                        </option>
                        <option value="pdfImageOnly">
                          PDF - Images Only Report{" "}
                        </option>
                      </Input>
          ...
          <ModalFooter>
            <Button size="sm" disabled={loader2} outline onClick={closeModal}>
              Close
            </Button>
            <Button
              size="sm"
              color="success"
              disabled={loader2}
              onClick={submitReportGeneratin}
            >
              {loader2 ? "loading.." : "Create"}
            </Button>
          </ModalFooter>
        </ModalCom>
      )}
