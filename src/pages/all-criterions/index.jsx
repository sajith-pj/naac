...
function AllCriterions() {
  ...
  const [selectedCriteria, setSelectedCriteria] = useState(
    localStorage.getItem("selected_criterion") || "1"
  );
  ...
  let selectedSubCriterionFromLocal = localStorage.getItem(
    "selected_sub_criterion"
  );

  const sortCriterions = (sub_h) => {
    let arr = sub_h;
    let temp;
    for (let i = 0; i < arr.length - 1; i++) {
      for (let j = i + 1; j <= arr.length - 1; j++) {
        if (arr[i].split(".")[1] == arr[j].split(".")[1]) {
          if (parseInt(arr[i].split(".")[2]) > parseInt(arr[j].split(".")[2])) {
            temp = arr[j];
            arr[j] = arr[i];
            arr[i] = temp;
          }
        } else if (arr[i].split(".")[1] > arr[j].split(".")[1]) {
          temp = arr[j];
          arr[j] = arr[i];
          arr[i] = temp;
        }
  ...
  useEffect(() => {
    if (criteriaList.length > 0) {
      ...
      if (selectedCriterionFromLocal) {
        crite = criteriaList.filter(
          (val) => val.main_id === selectedCriterionFromLocal
        );
      } else {
      ...
      data.forEach((val) => {
        if (val?.criterion) {
          if (user.user_scope === "STAFF" || user.user_scope === "CLUB") {
      ...
      setSubCriteriaHead(sortCriterions([...sub_h]));
      ...
      .then((res) => {
        ...
        sub_c.forEach((val) => {
          if (val?.criterion) {
            ...
            } else if (!sub_h.includes(val.criterion))
              sub_h.push(val.criterion);
          }
        });
  ...
  useEffect(() => {
    if (selectedCriteriaHead) {
      ...
      data.forEach((val) => {
        let sub =
          val?.sub_criterion?.filter(
            (i) => i.criterion === selectedCriteriaHead
          ) || [];
        if (sub.length > 0) {
          arr = sub?.[0] || {};
        }
  ...
  const getSelectedCriterionDetails = () => {
    setTableLoader(true);
    axios
      .get(`/criterion/data/list-create`, {
        params: {
          criterion_id: `c${selectedCriteriaHead.replaceAll(".", "_")}`,
        },
      })
      .then((response) => {
    ...
    axios
      .get(
        `/criterion/add-button/c${selectedCriteriaHead.replaceAll(".", "_")}`
      )
      .then((response) => {
        ...
        <Col xs="12" className="mt-2">
          ...
          <div className="creteria-head">
            {subCriteriaHead.map((val, ky) => {
              return (
                <button
                  onClick={() => {
                    setSelectedCriteriaHead(val);
                    localStorage.setItem("selected_criterion", "");
                    localStorage.setItem("selected_sub_criterion", "");
                  }}
                  className={`creteria-head-box ${
                    selectedCriteriaHead === val ? "active" : ""
                  }`}
                  key={ky}
                >{`C ${val}`}</button>
              );
          ...
          <div className="criterion-mobile">
            ...
            <Input
              ...
              type="select"
              onChange={(e) => {
                setSelectedCriteriaHead(e.target.value);
                localStorage.setItem("selected_criterion", "");
                localStorage.setItem("selected_sub_criterion", "");
              }}
              value={selectedCriteriaHead}
            >
              {subCriteriaHead.length > 0
                ? subCriteriaHead.map((val, ky) => {
                    return (
                      <option key={ky} value={val}>
                        {val}
                      </option>
                    );
                  })
                  ...
                  <tr key={ky} style={{ cursor: "auto" }}>
                    ...
                    <td>
                      <VisibilityIcon
                        ...
                        onClick={() => {
                          viewCriterion(val.id);
                        }}
                      />
                      {!["HOD", "CLUB", "STAFF"].includes(
                        user?.user_scope
                      ) ? (
                        <EditIcon
