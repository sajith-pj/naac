...
function ViewCriterion() {
  ...
  const getCriterionDetails = () => {
    axios
      .get(`/criterion/data/show/${params.id}`)
      .then((response) => {
  ...
  const changeStatusApiCall = (status, message) => {
    setBtnLoader(true);
    axios
      .post(`/criterion/change-status/${params.id}`, {
        status_to: status,
        message,
      })
      .then((response) => {
  ...
  const getDeleteButton = () => {
    if (
      ["ADMIN", "IQAC_COD"].includes(user?.user_scope) ||
      user?.id === criterionData?.created_by?.id
    ) {
  ...
  const getVerifyButton = () => {
    if (
      ...
      if (criterionData?.is_deleted === false && user?.user_scope === "HOD") {
        return (
          <Button
            type="button"
            size="sm"
  ...
  const getApproveButton = () => {
    if (user?.user_scope === "ADMIN" || user?.user_scope === "IQAC_COD") {
      if (
        criterionData?.is_deleted === false &&
        (criterionData?.status === "PENDING" ||
          criterionData?.status === "VERIFIED")
  ...
  return (
    <Row>
      <div className="pagetitle">
        ...
        <nav>
          <ol className="breadcrumb">
            ...
            <li className="breadcrumb-item active">View Criteria</li>
          </ol>
        </nav>
      </div>
          ...
          <CardBody>
            <Table>
              ...
              <tbody>
                ...
                <tr>
                  <td>Batch</td>
                  <td>{criterionData?.batch?.name || "-"}</td>
                </tr>
                <tr>
                  <td>Paper</td>
                  <td>{criterionData?.paper?.name || "-"}</td>
                </tr>
                <tr>
                  <td> Program </td>
                  <td>{criterionData?.program?.name || "-"}</td>
                </tr>
                <tr>
                  <td> Department </td>
                  <td>{criterionData?.department?.name || "-"}</td>
                </tr>
                {Object.entries(criterionData?.jdoc)?.length > 0
                  ? Object.entries(criterionData?.jdoc).map((field) => {
                      let extractedFields = field[1];
                      return (
                        <tr>
                          <td>{extractedFields[0]}</td>
                          <td>
                            {typeof extractedFields[1] === "boolean"
                              ? extractedFields[1]
                                ? "Yes"
                                : "No"
                              : getFileName(extractedFields[1])}
                          </td>
                        </tr>
                      );
                    })
                ...
                <tr>
                  <td> User </td>
                  <td>{criterionData?.created_by?.username}</td>
                </tr>
                <tr>
                  <td> Status </td>
      ...
      <ModalCom open={revertModal} title={"Alert !"} toggle={closeModal}>
        ...
        <ModalFooter>
          <Button size="sm" onClick={closeModal} disabled={Btnloader}>
            Cancel
          </Button>
          <Button
            size="sm"
            onClick={() => {
              changeStatus("REVERTED", revertMessage);
            }}
            disabled={!revertMessage || Btnloader}
            color="warning"
          >
            Revert
          </Button>
