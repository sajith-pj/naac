...
import { DESIGNATION, USER_SCOPES } from "../../utils/constant";
...
function UserList() {
  ...
  const getDepartmentList = () => {
    axios.get("/features").then((response) => {
      ...
      setClubsList(response?.data?.data.clubs || []);
    });
  };
  ...
  const updateAll = (type) => {
    setLoader(true);
    axios
      .post(`/user/all-status`, {
        status: type === "activate",
      })
      .then((response) => {
        toast.success(
          `${
            type === "activate" ? "Activated" : "Disabled"
          } all user successfully`,
          {
            position: "top-right",
            autoClose: 2000,
          }
  ...
  return (
    <Row>
      <div className="pagetitle">
        <h1>All User List</h1>
        <h6>
          List of registered users with scope as 'END USER', (Both Activated and
          Pending)
        </h6>
      </div>
      <Col xs="12" className="d-flex justify-content-end">
        <Button
          onClick={() => updateAll("activate")}
          className="me-2"
          outline
          color="success"
          bssize="sm"
      ...
      <ModalCom
        open={open}
        title={`Update details of ${userData?.username}`}
        toggle={() => {
          setOpen(false);
          setUserdata("");
          setErr({});
          ...
          <Row>
            ...
            <Col xs="12" sm="6">
              <FormGroup>
                ...
                <Input
                  name="user_scope"
                  type="select"
                  value={userData?.user_scope}
                  onChange={updateHandleChange}
                  invalid={!!err.user_scope}
                  defaultValue=""
                >
                  <option value="">User Scope</option>
                  {USER_SCOPES.map((scope, ky) => (
                    <option key={ky} value={scope.value}>
                      {scope.value}
                    </option>
                  ))}
                </Input>
              ...
              <Col xs="12" sm="12">
                <FormGroup>
                  ...
                  >
                    ...
                    {clubsList.map((club, ky) => (
                      <option key={ky} value={club.id}>
                        {club.name}
                      </option>
                    ))}
                  </Input>
            ...
            <Col xs="12" sm="6">
              <Input
                ...
                onChange={updateHandleChange}
              />{" "}
              <Label>Permission to Switch Role</Label>
            </Col>
          </Row>
        </ModalBody>
        <ModalFooter>
