...
function Settings() {
  ...
  const validateForm = () => {
    let obj = {
      ...
      admin_url: settingsData?.admin_url
        ? validateURL(settingsData?.admin_url)
        : "",
    };
  ...
  const submitSeting = () => {
    ...
    if (changeSettings && settingsData?.id) {
      setSubmitting(true);
      axios
        .put(`/adminapp/settings/details/${settingsData?.id}`, settingsData, {
          headers: {
      ...
      setSubmitting(true);
      axios
        .post("/adminapp/settings/list", settingsData, {
          headers: {
            "Content-Type": "application/json",
          },
        })
  ...
  return (
    <Row>
      <div className="pagetitle">
        ...
        {showTable && (
          <h6>The settings are already exist and can be changed by user</h6>
        )}
        ...
        <>
          <Col xs="12" sm="9">
            <Row>
              ...
              <Col xs="12" sm="6" className="mt-2">
                <Label>Portal name (App title)</Label>
                <Input
                  type="text"
                  name="title"
                  id="title"
              ...
              <Col xs="12" sm="6" className="mt-2">
                ...
                <Input
                  ...
                  id="input_version"
                  onChange={handleChange}
                  value={settingsData?.input_version}
                  invalid={!!err.input_version}
                >
                  <option value=""> Select Input Version</option>
                  {versionList.map((version) => (
                    <option value={version}>{version}</option>
                  ))}
                </Input>
                <FormFeedback>{err?.input_version}</FormFeedback>
              </Col>
              <Col xs="12" sm="6" className="mt-2">
                ...
                <Input
                  ...
                  id="report_version"
                  onChange={handleChange}
                  value={settingsData?.report_version}
                  invalid={!!err.report_version}
                >
                  <option value=""> Select Input Version</option>
                  {versionList.map((version) => (
                    <option value={version}>{version}</option>
                ...
                <FormFeedback>{err?.report_version}</FormFeedback>
              </Col>
              <Col xs="12" sm="6" className="mt-2">
          ...
          <Col xs="12" sm="3" className="d-flex flex-column mt-2">
            <FormGroup>
              <Input
                ...
                checked={settingsData?.allow_signup}
              />
              {"  "}
            ...
            <FormGroup>
              <Input
                type="checkbox"
                name="allow_data_entry"
                onChange={handleCheck}
                checked={settingsData?.allow_data_entry}
              />
              {"  "}
              <Label>Enable data entry</Label>
            </FormGroup>
            <FormGroup>
              <Input
                type="checkbox"
                name="allow_change_roles"
                onChange={handleCheck}
                checked={settingsData?.allow_change_roles}
              />
              {"  "}
              <Label>Allow change roles</Label>
            </FormGroup>
            <FormGroup>
              <Input
                type="checkbox"
                name="enable_otp"
                onChange={handleCheck}
                checked={settingsData?.enable_otp}
              />
              {"  "}
            ...
            <FormGroup>
              <Input
                ...
                name="enable_captha"
                onChange={handleCheck}
                checked={settingsData?.enable_captha}
              />
              {"  "}
              <Label>Enable Re-Captha verification</Label>
            </FormGroup>
            <FormGroup>
              <Input
                type="checkbox"
                name="enable_https"
                onChange={handleCheck}
                checked={settingsData?.enable_https}
              />
              {"  "}
              <Label>Enable HTTPS (Force)</Label>
            </FormGroup>
            <FormGroup>
              <Input
                type="checkbox"
            ...
            <FormGroup>
              <Input
                ...
                name="classic_ui"
                onChange={handleCheck}
                checked={settingsData?.classic_ui}
              />
              {"  "}
            ...
            <FormGroup>
              <Input
                ...
                name="is_app_active"
                onChange={handleCheck}
                checked={settingsData?.is_app_active}
              />
              {"  "}
