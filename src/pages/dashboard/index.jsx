    ...
    <Row>
          ...
          <CardBody style={{ padding: "5px" }}>
            <div className="notice-div">
              {noticeBoardList.length > 0
                ? noticeBoardList.map((val, ky) => {
                    return (
                      <div
                        className="notice-item"
                        key={ky}
                        onClick={() => (val?.urls ? window.open(val.urls) : "")}
                      >
      ...
      <Col xs="12" sm="12">
        <Card>
          ...
          <CardBody>
            <Row>
              {userDetails.user_scope !== "ADMIN" &&
              userDetails.user_scope !== "IQAC_COD" ? (
                dashboardData?.dashboard_summury?.map((summary, index) => {
                  if (summary[Object.keys(summary)[0]].assigned) {
                    return (
                      <Col xs="12" sm="3">
                        <Card
                          className="dashboard-criterion-cards"
                          onClick={() =>
                            navigateToCriterion({
                              criterion: Object.keys(summary)[0],
                            })
                          }
                        >
                          <CardHeader>
                          ...
                          <CardBody>
                            <div className="dashboard-summary-body">
                              <p>
                                {" "}
                                Criterion Attended:{" "}
                                {summary[Object.keys(summary)[0]].attended}{" "}
                              </p>
                              <p>
                                {" "}
                                Criterion Pending:{" "}
                                {summary[Object.keys(summary)[0]].pending}{" "}
                              </p>
                              <p>
                                {" "}
                                Criterion assigned:{" "}
                                {summary[Object.keys(summary)[0]].question}{" "}
                              </p>
                            </div>
                          </CardBody>
                        </Card>
                ...
                <>
                  <Col xs="12" sm="6" md="4">
                    <div className="dash-box first">
                      <div className="count">
                        {dashboardData?.total_active_criterions}
                      </div>
                  ...
                  <Col xs="12" sm="6" md="4">
                    <div className="dash-box second">
                      <div className="count">
                        {dashboardData?.assigned_criterions}
                      </div>
