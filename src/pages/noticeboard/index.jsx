...
function NoticeBoard() {
  ...
  const column = [
    {
      ...
      render: (rowData) => {
        return (
          <>{rowData?.created_at ? rowData?.created_at.slice(0, 10) : ""}</>
        );
      },
    },
          ...
          <>
            {rowData?.urls ? (
              <a href={rowData.urls} target="_blank" rel="noopener noreferrer">
                <p className="view_p">Visit URL</p>{" "}
              </a>
            ) : (
              "-"
            )}
          ...
          <>
            {rowData?.files ? (
              <a href={rowData.files} target="_blank" rel="noopener noreferrer">
                <p className="view_p">View File</p>{" "}
              </a>
            ) : (
              "-"
            )}
  ...
  const submiNotice = () => {
    ...
    if (Object.values(errObj).some((val) => val !== "")) {
      ...
      axios
        .put(`/programs/noticeboard/details/${inputValues?.id}`, formData)
        .then((response) => {
      ...
      axios
        .post("/programs/noticeboard/list", formData)
        .then((response) => {
  ...
  const noticeBoardDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setDeleting(true);
        axios
          .delete(`/programs/noticeboard/details/${inputValues.id}`, {
            headers: {
  ...
  return (
    <Row className="faq">
      <div className="pagetitle">
        ...
        <nav>
          <ol className="breadcrumb">
            ...
            <li className="breadcrumb-item active">Noticeboard</li>
          </ol>
        </nav>
      </div>
      ...
      <ModalCom
        open={open}
        title={`${inputValues?.id ? "Update Notice" : "Add Notice"}`}
        toggle={closeModal}
      >
        <ModalBody>
          <Row>
            ...
            <Col xs="12" sm="6">
              <Input
                ...
                name="link_to"
                id="link_to"
                checked={inputValues?.link_to}
                onChange={handleChange}
              />
              <Label htmlFor="link_to">Link to URL</Label>
            </Col>
          </Row>
        </ModalBody>
        <ModalFooter>
