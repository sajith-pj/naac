...
function Batch() {
  ...
  const submitBatch = () => {
    ...
    if (Object.values(errObj).some((val) => val !== "")) {
      ...
      setUpdating(true);
      axios
        .put(`/programs/batch/details/${inputValues?.id}`, inputValues)
        .then((response) => {
      ...
      setUpdating(true);
      axios
        .post("/programs/batch/list", inputValues)
        .then((response) => {
  ...
  return (
    <Row>
      ...
      <ModalCom
        open={open}
        title={`${inputValues?.id ? "Update Batch" : "Add Batch"}`}
        toggle={closeModal}
      >
        <ModalBody>
          <Row>
            ...
            <Col xs="12" sm="6">
              <FormGroup>
                ...
                <Input
                  ...
                  min={0}
                  name="batch_no"
                  value={inputValues?.batch_no}
                  onChange={handleChange}
                  invalid={!!err.batch_no}
                />
