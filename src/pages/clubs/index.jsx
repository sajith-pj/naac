...
function Clubs() {
  ...
  const deleteClub = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setUpdating(true);
        axios
          .delete(`/programs/clubs/details/${inputValues?.id}`, {})
          .then(() => {
  ...
  const submitClub = () => {
    ...
    if (Object.values(errObj).some((val) => val !== "")) {
      ...
      axios
        .put(`/programs/clubs/details/${inputValues?.id}`, {
          ...rest,
          user: userId,
        })
        .then((response) => {
      ...
      axios
        .post("/programs/clubs/list", { ...inputValues, user: userId })
        .then((response) => {
  ...
  const handleAssignChange = (e) => {
    ...
    if (e.target.value) {
      setFetching(true);
      axios
        .get(`/programs/clubs/details/${e.target.value}`)
        .then((response) => {
  ...
  return (
    <Row>
      ...
      <ModalCom
        open={open}
        title={`${inputValues?.id ? "Update Club" : "Add Club"}`}
        toggle={closeModal}
      >
        <ModalBody>
          <Row>
            ...
            <Col xs="12" sm="6">
              <FormGroup>
                ...
                <Input
                  type="date"
                  name="established_at"
                  value={inputValues?.established_at}
                  onChange={handleChange}
                  invalid={!!err.established_at}
                />
      ...
      <ModalCom
        open={openAssign}
        title={"Assign Users to Club"}
        toggle={closeAssignUser}
      >
        <ModalBody>
          <Row>
            <Col xs="12">
        ...
        <ModalFooter>
          <Button
            color="primary"
            size="sm"
            disabled={assigning || !selectedClub}
            onClick={assignUsersToClub}
          >
