...
function Student() {
  ...
  const deleteStudent = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setUpdating(true);
        axios
          .delete(`/programs/students/details/${inputValues?.id}`, {})
          .then(() => {
  ...
  const submitPaper = () => {
    ...
    if (Object.values(errObj).some((val) => val !== "")) {
      ...
      setUpdating(true);
      axios
        .put(`/programs/students/details/${inputValues?.id}`, inputValues)
        .then((response) => {
      ...
      setUpdating(true);
      axios
        .post("/programs/students/list", inputValues)
        .then((response) => {
  ...
  return (
    <Row>
      ...
      <ModalCom
        open={open}
        title={`${inputValues?.id ? "Update Student" : "Add Student"}`}
        toggle={closeModal}
      >
        <ModalBody>
          <Row>
            ...
            <Col xs="6">
              ...
              />
              {"  "}
              <Label>Enabled (System status)</Label>
            </Col>
