...
function Faq() {
  ...
  const submitFaq = () => {
    ...
    if (Object.values(errObj).some((val) => val !== "")) {
      ...
      axios
        .put(`/programs/faq/details/${inputValues?.id}`, formData)
        .then((response) => {
      ...
      axios
        .post("/programs/faq/list", formData)
        .then((response) => {
  ...
  return (
    <Row className="faq">
      ...
      <Col xs="12" className="d-flex justify-content-end">
        {!["HOD", "CLUB", "STAFF"].includes(user?.user_scope) && (
          <Button
            onClick={() => addFaq()}
            className="me-2"
            outline
                ...
                <AccordionBody accordionId={k}>
                  {!["HOD", "CLUB", "STAFF"].includes(
                    ...
                    <div className="edit-faq">
                      <span onClick={() => editFaq(val)}>Edit Faq</span>
                    </div>
                  )}
                  <p>{val?.answer}</p>
                  <div className="faq-attaches">
                    {val?.image ? (
                      ...
                      >
                        <p className="naac-faq-links"> View attached Images </p>{" "}
                      </a>
                    ) : (
                      ""
                    )}
                    {val?.files ? (
                      <a
                        ...
                        rel="noopener noreferrer"
                      >
                        <p className="naac-faq-links"> View attached Files </p>{" "}
                      </a>
                    ) : (
                      ""
                    )}
                    {val?.urls ? (
                      <a
                        ...
                        rel="noopener noreferrer"
                      >
                        <p className="naac-faq-links"> View External Link </p>{" "}
                      </a>
                    ) : (
                      ""
                    )}
      ...
      <ModalCom
        open={open}
        title={`${inputValues?.id ? "Update FAQ" : "Add FAQ"}`}
        toggle={closeModal}
      >
        <ModalBody>
          <Row>
            ...
            <Col xs="12">
              <FormGroup>
                ...
                {files?.image && inputValues?.id && (
                  <a
                    href={files.image}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <p className="view_p">View Current Image</p>
                  </a>
                )}
              </FormGroup>
            </Col>
            ...
            <Col xs="12">
              <FormGroup>
                ...
                {files?.files && inputValues?.id && (
                  <a
                    href={files.files}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <p className="view_p">View Current File</p>
                  </a>
                )}
              </FormGroup>
            </Col>
            ...
            <Col xs="12">
              <Input
                type="checkbox"
                name="is_enabled"
                checked={inputValues?.is_enabled}
                onChange={handleChange}
              />
              {"  "}
              <Label>Enable For User</Label>
            </Col>
          </Row>
        </ModalBody>
        <ModalFooter>
